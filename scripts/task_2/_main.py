...
def evaluate(data_source):
    ...
    ntokens = len(corpus.dictionary)
    if args.model != 'Transformer':
        hidden = mymodel.init_hidden(eval_batch_size)
    with torch.no_grad():
        for i in range(0, data_source.size(0) - 1, args.bptt):
            ...
            if args.model == 'Transformer':
                ...
                output, hidden = mymodel(data, hidden)
...
def train(mymodel, drpt):
    ...
    ntokens = len(corpus.dictionary)
    if args.model != 'Transformer':
        hidden = mymodel.init_hidden(args.batch_size)
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        torch.nn.utils.clip_grad_norm_(mymodel.parameters(), args.clip)
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            if args.ppl_log:
                training_log[drpt].append((epoch, math.exp(cur_loss)))
            total_loss = 0
            start_time = time.time()
        

# TODO remove not to deactivate default dryrun
        if not args.dry_run:
            break
...
def export_onnx(path, batch_size, seq_len):
    ...
    torch.onnx.export(mymodel, (dummy_input, hidden), path)
...
try:
    for drpt, mdl in models_drpout.items():
        ...
        for epoch in range(1, args.epochs+1):
            ...
            if args.ppl_log:
                valid_log[drpt].append((epoch, math.exp(val_loss)))
...
print(f'validation log: {valid_log}')
