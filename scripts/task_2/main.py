...
for dropout in args.dropout:
    ...
    if args.model == 'Transformer':
        ...
        mymodel = model.RNNModel(args.model, ntokens, args.emsize, args.nhid, args.nlayers, dropout, args.tied).to(device)
...
def train(modl):
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        torch.nn.utils.clip_grad_norm_(modl.parameters(), args.clip)
        for p in modl.parameters():
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            if args.ppl_log:
                ...
                training_log[modl.dropoutvalue].append(math.exp(cur_loss))
    ...
    for drpt, mdl in models_drpout.items():
        ...
        for epoch in range(1, args.epochs+1):
            ...
            if args.ppl_log:
                ...
                # validation_log[drpt].append((epoch,math.exp(val_loss)))
            # Save the model if the validation loss is the best we've seen so far.
            if not best_val_loss or val_loss < best_val_loss:
                # SAVING MULTIPLE MODELS 
                if args.save.endswith('.pt'): name = f'{args.save[:-3]}_drpt{drpt}{args.save[-3:]}'
                else: name = f'{args.save}_drpt{drpt}.pt'
                ...
                lr /= 4.0
        # reset dynamic parameters to defaults/ridiculously high numbers
...
# with open('validation.txt', 'w', encoding='utf-8') as val, open('training.txt', 'w', encoding='utf-8') as trn, open('testing.txt', 'w', encoding='utf-8') as tst:
#     val.write(str(validation_log))
#     trn.write(str(training_log))
#     tst.write(str(test_log))
